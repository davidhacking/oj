...
/*
dp, f(i, j) = (m1, m2) m1到达(i, j)所剩余的生命值，m2到达(i, j)所需要的最小初始化生命值
显然m1 >= 1
这是一个错误答案呵呵，[[0,0,0],[-1,0,0],[2,0,-2]]这个例子就不行，想不到怎么正过来dp就倒过来
*/
...
struct Data
{
	int m1;
	int m2;
	Data(int m1, int m2): m1(m1), m2(m2) {}
};
...
public:
    ...
    int calculateMinimumHP(vector<vector<int> >& dungeon) {
        ...
        for (int i = 0; i < m; i++) {
        	f.push_back(vector<Data>());
        	for (int j = 0; j < n; j++) {
        		f[i].push_back(Data(0, 0));
        	}
        }
        if (dungeon[0][0] >= 1) {
        	f[0][0].m1 = dungeon[0][0] + 1;
        	f[0][0].m2 = 1;
        } else {
        	f[0][0].m1 = 1;
        	f[0][0].m2 = 1 - dungeon[0][0];
        }
        for (int j = 1; j < n; j++) {
        	if (dungeon[0][j] + f[0][j - 1].m1 >= 1) {
	        	f[0][j].m1 = dungeon[0][j] + f[0][j - 1].m1;
	        	f[0][j].m2 = f[0][j - 1].m2;
	        } else {
	        	f[0][j].m1 = 1;
	        	f[0][j].m2 = 1 - (dungeon[0][j] + f[0][j - 1].m1) + f[0][j - 1].m2;
	        }
        }
        for (int i = 1; i < m; i++) {
        	if (dungeon[i][0] + f[i - 1][0].m1 >= 1) {
	        	f[i][0].m1 = dungeon[i][0] + f[i - 1][0].m1;
	        	f[i][0].m2 = f[i - 1][0].m2;
	        } else {
	        	f[i][0].m1 = 1;
	        	f[i][0].m2 = 1 - (dungeon[i][0] + f[i - 1][0].m1) + f[i - 1][0].m2;
	        }
        }
        for (int i = 1; i < m; i++) {
        	for (int j = 1; j < n; j++) {
        		int m11, m12, m21, m22;
        		if (dungeon[i][j] + f[i - 1][j].m1 >= 1) {
		        	m11 = dungeon[i][j] + f[i - 1][j].m1;
		        	m12 = f[i - 1][j].m2;
		        } else {
		        	m11 = 1;
		        	m12 = 1 - (dungeon[i][j] + f[i - 1][j].m1) + f[i - 1][j].m2;
		        }
		        if (dungeon[i][j] + f[i][j - 1].m1 >= 1) {
		        	m21 = dungeon[i][j] + f[i][j - 1].m1;
		        	m22 = f[i][j - 1].m2;
		        } else {
		        	m21 = 1;
		        	m22 = 1 - (dungeon[i][j] + f[i][j - 1].m1) + f[i][j - 1].m2;
		        }
		        if (m12 < m22) {
		        	// if (m22 - m11 < m21 - m12) {
